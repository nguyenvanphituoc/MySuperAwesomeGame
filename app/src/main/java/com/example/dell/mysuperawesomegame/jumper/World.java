...
public class World {
    ...
    private void generateLevel() {
        ...
        float maxJumpHeight =(float) Bob.BOB_JUMP_VELOCITY * (float)Bob.BOB_JUMP_VELOCITY
                / (2 * -(float)gravity.y);
        while (y < WORLD_HEIGHT - WORLD_WIDTH / 2) {
            ...
            if (rand.nextFloat() > 0.9f
                ...
                Spring spring = new Spring((float)platform.position.x,
                        (float)platform.position.y + Platform.PLATFORM_HEIGHT / 2
                                + Spring.SPRING_HEIGHT / 2);
                springs.add(spring);
            }
            if (y > WORLD_HEIGHT / 3 && rand.nextFloat() > 0.8f) {
                Squirrel squirrel = new Squirrel((float)platform.position.x
                        + rand.nextFloat(), (float)platform.position.y
                        + Squirrel.SQUIRREL_HEIGHT + rand.nextFloat() * 2);
                squirrels.add(squirrel);
            }
            if (rand.nextFloat() > 0.6f) {
                Coin coin = new Coin((float)platform.position.x + rand.nextFloat(),
                        (float)platform.position.y + Coin.COIN_HEIGHT
                                + rand.nextFloat() * 3);
                coins.add(coin);
            }
    ...
    private void updateBob(float deltaTime, float accelX) {
        ...
        heightSoFar = (float)Math.max(bob.position.y, heightSoFar);
    }
